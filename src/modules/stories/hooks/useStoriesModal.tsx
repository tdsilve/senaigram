...
export const useStoriesModal = () => {
    ...
    const { dispatch, modal: { userId, userName } } = useStoriesContext();
    console.log(userId)
    
    const [inPause, setInPause] = React.useState(false);
    const initialStories = STORIES[userId]?.stories;
    const storiesInitialValue = getCurrentStoriesInitialConfig(initialStories, userId);

    const [storiesState, storiesDispatch] = React.useReducer(storiesModalReducer, storiesInitialValue);
    ...
    const { avatar: userAvatar } = USERS.find(user => user.name === userName) as Users;
