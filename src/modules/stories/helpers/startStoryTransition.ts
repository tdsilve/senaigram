import { STORIES_MODAL_REDUCER_ACTIONS, STORIES_REDUCER_ACTIONS } from "../types/enums"
    ...
    const { 
      ...
      if(!storiesRemaining) {
        ...
        if(moreUsersStories){
          ...
          dispatch?.({ type: STORIES_REDUCER_ACTIONS.SET_MODAL, config: {
            userId: userId + 1,
            userName
          }})
        }
    
        if(!moreUsersStories) toggleModal(dispatch as React.Dispatch<Payload>, config)
